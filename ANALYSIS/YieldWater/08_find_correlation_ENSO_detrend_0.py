...
out_dir = rf"D:\Malaysia\02_Timeseries\YieldWater\06_correlation_timelag_ENSO\{pp}"
    ...
    for var in varlist:    
        ...
        for mon in range(1,13,1):
            ...
            for yr in years_region:
                ...
                var_yrt = df_csv_m_yrt[f"{var}"].values[0] # This is X at year t, month m  #f"{var}z"
                try:
                    var_yrt_1 = df_csv_m_yrt_1[f"{var}"].values[0] # This is X at year t-1, month m  #f"{var}z"
                except:
                    var_yrt_1 = np.nan
...
for i, row in tqdm(gdf_region.iterrows()):
    ...
    if reginame in nondata_region:
        continue
    # else:
    #     check = out_dir + os.sep + f"pearson_{regioname_fin}_stdabs.csv"
    ...
    fig,axes = plt.subplots(len(varlist),1, figsize=(10, 8))
    ...
    for i,var in enumerate(varlist):
        ...
        ax.set_ylabel(f"Pearson_{var} index", fontsize = 25)
