...
startyear = 2002
endyear = 2022
out_dir = r"D:\Malaysia\02_Timeseries\Resilience\03_change_point"
...
# sd_dict = {}
# ampli_dict ={}
results = {"NumChangePoints":{}, "trendDiff":{}, "stdDiff":{}, "amplitudeDiff":{}}
...
for csvfile in tqdm(csvs):
    ...
    if len(nan_chck) ==0: # no valid area
        ...
        # jump controls the grid of possible change points e.g. k, 2k, 3k...
        ## Negative log likelihoodをcustom codingした方がBICとするなら論理的に正しそう
        # https://stackoverflow.com/questions/71920414/change-point-detection-pelt
        algo = rpt.Pelt(model="normal", min_size=60).fit(data_sif)
        ## Penaty
        ## BIC: https://stackoverflow.com/questions/60823638/python-function-for-bayesiasn-information-criterion-bic
        pena = 2*math.log(len(data_sif))
        
        ## seems make error when data length too short
        try:        
            my_bkps  = algo.predict(pen=pena)
            """
            fig, ax_arr = rpt.display(data_sif, my_bkps , figsize=(10, 6))
            plt.show()
            """
            out_dir_fin = out_dir + os.sep + "_png"
            outfilename = out_dir_fin + os.sep + f"changepoint_{filename}.png"
            # fig.savefig(outfilename)
            
            ###  # exclude last change timing (I don't know why included)
            last_time = len(data_sif)
            my_bkps_fin = [m for m in my_bkps if m != last_time]
            
            ## input num_chpoints
            results["NumChangePoints"][filename] = len(my_bkps_fin)
        except:
            my_bkps = [] #half period, instead
            my_bkps_fin = []
            ## input num_chpoints
            results["NumChangePoints"][filename] = 0
        
        
        """ # identify change date"""
        if len(my_bkps_fin) ==0: # no change point
            # results["trendLast"][filename] = np.nan
            # results["stdDiff"][filename] = np.nan
            # results["amplitudeDiff"][filename] = np.nan
            # continue
...
for ky, valdict in results.items():
    ...
    out_file = out_dir + os.sep +f"{page}_{ky}_PELT.tif"
