# -*- coding: utf-8 -*-
"""
PCA and PCR by including Yt-1 as autocorrelation, i.e. ARX 
subtract monthly mena and z scored
"""
...
def main(csvfile, pval, startyear, endyear, time_lag):
    # print(csvfile)
    ...
    """ #(pixel単位で)月ごとに平均引いて(anomaly)、その後に全体でzscoring""" 
    df_csv_z = df_csv.copy()
    ...
    for var in vars_list:
        ...
        for m in months:
            ...
            ##インデックスで抽出して元のデータフレームの新規列にzscoreを入れる
            specific_month_idx = specific_month_rows.index.tolist()
            df_csv_z.loc[specific_month_idx, f"{var}tmp"] = df_csv_z.loc[specific_month_idx, var]-monthly_mean
        
        var_mean = df_csv_z.loc[:,var+"tmp"].mean()
        var_std = df_csv_z.loc[:,var+"tmp"].std()
        df_csv_z[f"{var}z"] = (df_csv_z[var+"tmp"]-var_mean)/var_std
        
    vars_list_tmp = [v + "tmp" for v in vars_list]
    df_csv_z = df_csv_z.drop(vars_list_tmp, axis=1)
    ...
    ### nan削除
    df_valid = df_csv_z.dropna(how="any") #nanがあれば除外
