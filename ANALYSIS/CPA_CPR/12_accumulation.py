...
for page in ["A1","A2","A3","A4"]:
    ...
    out_dir = "/Volumes/SSD_2/Malaysia/02_Timeseries/CPA_CPR/7_accumulation" + os.sep + f"accum_{str(time_lag)}"
    ...
    for csvfile in tqdm(csv_file_list):
        ...
        if len(df_valid) ==0:
            ...
            """　# accum vars for accum time """     
            # reset datetime index
            df_valid_reset = df_valid.reset_index()
            
            # extract sif
            df_sif = df_valid_reset.loc[:,["GOSIFz"]]
            # extract vars
            vars_listz = [v + "z" for v in vars_list if v != "GOSIF"]
            df_vars = df_valid_reset.drop(["GOSIFz","datetime"], axis=1)
            ...
            # concat rolled vars and sif
            df_valid_lag = pd.concat([df_sif, df_vars_accum], axis=1)
    ...
    for vari in use_vars:
        ...
        for k, dicval in {"pearson":r_dic, "pval":pval_dic}.items():
            ...
            out_file = out_dir +os.sep +f"{page}_{vari}_{k}_accum{time_lag}_{str(startyear)}-{str(endyear)}.tif"
