# -*- coding: utf-8 -*-
"""
# apply best lag and accumlation dataset for best coef
"""
...
# csvfile = r"F:\MAlaysia\ANALYSIS\02_Timeseries\CPA_CPR\1_vars_at_pixels\A1\7443.csv"
# startyear = 2002
# endyear = 2012
# pval=0.1
# pagenum = "A1"

best_lag_dir = r"D:\Malaysia\02_Timeseries\CPA_CPR\6_time_lag\lag_best"
best_accum_dir = r"D:\Malaysia\02_Timeseries\CPA_CPR\7_accumulation\accum_best"

def main(csvfile, pval, startyear, endyear, pagenum):
    
    def raster_arr_idx_val(tifpath,idxnum):
        with rasterio.open(tifpath) as src:
            arr = src.read(1)
            arr_1d = np.ravel(arr)
            arr_idx_val = arr_1d[idxnum]
            return arr_idx_val
    ...
    """　#Select best lag and accumlation """
    vars_list_only = [v for v in vars_list if v != "GOSIF"]
    
    var_time_effects = []
    for var in vars_list_only:
        tif_var_lag = glob.glob(best_lag_dir + os.sep + f"{pagenum}_{var}_pearson_best_{startyear}-{endyear}.tif")[0]
        tif_var_accum = glob.glob(best_accum_dir + os.sep + f"{pagenum}_{var}_pearson_best_{startyear}-{endyear}.tif")[0]
        
        arr_lag_1d = raster_arr_idx_val(tif_var_lag, file_index)
        arr_accum_1d = raster_arr_idx_val(tif_var_accum, file_index)
        
        df_var = df_csv_z.loc[:,var+"z"]
        ## accum first
        df_var_accum = df_var.rolling(int(arr_accum_1d)+1).sum()
        ## lagged
        df_var_lag = df_var_accum.shift(int(arr_lag_1d))
        
        var_time_effects.append(df_var_lag)
    
    ## concat with GOSIF
    df_valid_gosif = df_csv_z['GOSIFz']
    df_valid_effect = pd.concat([df_valid_gosif] + var_time_effects, axis=1)
    
    ### nan削除
    df_valid = df_valid_effect.dropna() #nanがあれば除外
    ...
    # importance_dic["p_values"] = model_sm.pvalues[1:].tolist() #間違い
    ...
    # A = np.identity(len(model_sm.params))
    # A = A[1:,:]
    # fstat = model_sm.f_test(A)
    # import scipy.stats as stats
    # p_value = stats.f.sf(1.8674822787476146, 4, 86) #これと .f_test(A)のp同じ
    
    importance_dic["p_values"] = model_sm.f_pvalue
